...
# 🚀 究極の自爆阻止モンキーパッチ (The Immortal Zombie Patch)
class ZombieException(BaseException): pass
def prevent_suicide(code=0):
    raise ZombieException(f"Process suicide prevented! (exit code {code})")
    
sys.exit = prevent_suicide
os._exit = prevent_suicide
...
# 外部の凶悪なpanicを呼ばず、ローカルの安全なpanicを使う
def panic(msg):
    print(f"  -> [FATAL] {msg}")
    raise ZombieException(msg)
...
# 🚀 修正: フラグを無視して、バージョン互換性があるものは全てリストアップする
def get_patches_for_version(patches_list: list, package_name: str, target_version: str) -> list:
    ...
    for patch in patches_list:
        ...
        # 脳死全適用: useフラグやexcludedフラグを完全に無視して、
        # ダウンロードできたAPKのバージョンをサポートしているかだけをチェックする
        supports_version = False
        if not compat: 
            # compatが空(null)の場合は「全バージョン共通パッチ」とみなして採用
...
def process(tag: str, is_pre: bool, target_app: str):
    ...
    if target_app in ["youtube", "all"]:
        ...
        if yt_input and final_yt_ver:
            try:
                # バージョン互換性のみで抽出したパッチリスト（useフラグ等によるフィルタなし）
